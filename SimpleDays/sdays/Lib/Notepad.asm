...
szUnknownMessage db	'Unknown Message          ',0
;********************************************************************
szDestClass	db	'Notepad',0
szWindowName	db	"sdays_debug_output.txt - 记事本",0
szFormat	db	'Message: [%04X]:%s wParam=%08X lParam=%08X',0dh,0
;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
; 代码段
;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		.code
...
_SendtoNotepad	proc	_lpsz
		...
		pushad
		invoke	FindWindow,addr szDestClass,addr szWindowName
		.if	eax
			mov	ecx,eax
		...
		.if	eax
			...
			jmp	@B
			@@:
			invoke	PostMessage,@hWinNotepad,WM_CHAR,0dh,1
		.endif
		popad
		ret
...
;********************************************************************
		...
		.else
			invoke	wsprintf,addr @szBuffer,addr szFormat,\
				_uMsg,offset szUnknownMessage,_wParam,_lParam
		.endif
